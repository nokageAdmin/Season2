...
if (isset($_POST["btnUpdate"])) {
    ...
    if (!$new_first_nameErr && !$new_middle_nameErr && !$new_last_nameErr && !$new_seven_digitErr && !$new_emailErr) {
        ...
        // Update the user data with new input values
        mysqli_query($connections, "UPDATE tbl_user SET
            ...
            gender = '$new_gender',
            preffix = '$new_preffix',
            seven_digit = '$new_seven_digit',
            email = '$new_email'
            WHERE id_user = '$id_user'");
    
        // Encrypt a unique identifier and set notification message
        $encrypted = md5(rand(1, 9));
        $notify_message = urlencode("Record has been updated!");
    
        // Redirect with the notification message in the URL
        echo "<script>window.location.href='ViewRecord.php?$encrypted&notify=$notify_message';</script>";
    }
